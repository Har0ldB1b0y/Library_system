...
class GenerateTransactionReport extends Job implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        $this->report->type = 'Transaction Report';
        $this->report->save();
...
//        }
        ...
        $pdf    = \PDF::loadView('admin.reports.transaction_pdf', compact('data'));
        ...
        Storage::makeDirectory('/reports/transaction-report/' . date('Y-m-d H:i:s') . " - {$this->report->id}");
        $final_filename = storage_path("app/reports/transaction-report/" . date('Y-m-d H:i:s') . " - {$this->report->id}/transaction_report{$this->report->id}.pdf");
